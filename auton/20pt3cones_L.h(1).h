...
AutoDriveRV = -5;
	...
	PIDArmTarget(1400);
	drive_stat=2;
	pid_Right_drive.PID_INTEGRAL_LIMIT = 10;
...
PIDArmTarget(1530);
	...
	mogoout();
	pid_left_drive.PID_INTEGRAL_LIMIT = 5;
	InitPIDDrive(3, 2, 1.9, 0); //with mogo 100
	GyroLeft(103,105);
	WFD();
	...
	while(SensorValue[UltraMogo]<13){
		wait1Msec(1);
	}
		AutoDriveLV = 0;
	AutoDriveRV = 0;
